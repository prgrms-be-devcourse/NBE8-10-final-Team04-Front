...
import { Copy, Terminal, Trash2, KeyRound } from "lucide-react";
...
export default function McpGuidePage() {
  ...
  const [form, setForm] = useState({
    name: "",
    expiresAt: format(addMonths(new Date(), 1), "yyyy-MM-dd"), // 기본 1개월 후
  });
  ...
  return (
    <PageLayout className="bg-slate-50 py-10 min-h-screen">
      <PageInner className="max-w-4xl">
        ...
        {/* 1. 마크다운 가이드 영역 */}
        <Card className="mb-10 overflow-hidden bg-white border-slate-200">
          ...
          <div className="p-6 md:p-8 overflow-x-auto">
            <ReactMarkdown
              ...
              components={{
                h2: ({node, ...props}) => (
                  <h2
                    className="text-2xl font-bold mt-6 mb-4 pb-2 border-b border-slate-200 text-slate-900"
                    {...props}
                  />
                ),
                ...
                ol: ({node, ...props}) => (
                  <ol className="list-decimal ml-6 mb-4 space-y-1 text-slate-700 marker:text-slate-400" {...props} />
                ),
                li: ({node, ...props}) => <li className="pl-1" {...props} />,
                blockquote: ({node, ...props}) => (
                  <blockquote
                    className="border-l-4 border-slate-300 pl-4 italic text-slate-600 bg-slate-50 py-3 pr-4 mb-4"
                    {...props}
                  />
                ),
                code({node, inline, className, children, ...props}: any) {
                  ...
                  return !inline && match ? (
                    <SyntaxHighlighter
                      ...
                      language={match}
                      PreTag="div"
                      className="rounded-xl !mt-2 !mb-6 text-sm border border-slate-800"
                      {...props}
                    >
                      {String(children).replace(/\n$/, "")}
                    </SyntaxHighlighter>
                  ) : (
                    <code className="bg-slate-100 text-pink-600 px-1.5 py-0.5 rounded font-mono text-sm" {...props}>
                      {children}
                    </code>
                  );
                },
              ...
              <tbody>
                {isLoading ? (
                  ...
                  tokens.map((token) => (
                    <tr
                      key={token.tokenId}
                      className={`border-b border-slate-100 \${token.revoked ? 'bg-slate-50 opacity-50' : 'hover:bg-slate-50'}`}
                    >
        ...
        {/* 3. 새 토큰 발급 완료 모달 (한 번만 보여짐) */}
        <Dialog open={!!newToken} onOpenChange={(open) => !open && setNewToken(null)}>
          <DialogContent className="max-w-md bg-white">
            <DialogHeader>
              <DialogTitle className="text-green-600">토큰 발급 성공!</DialogTitle>
            </DialogHeader>
            <div className="space-y-4 my-4">
              <div className="text-sm text-slate-600 space-y-1">
                <p>새 MCP 토큰이 발급되었습니다. 안전한 곳에 복사해 두세요.</p>
                <p>
                  <strong>이 토큰은 지금 한 번만 확인할 수 있습니다.</strong>
                </p>
              </div>
              <div className="p-3 bg-slate-100 rounded border border-slate-200 font-mono text-sm break-all">
                {newToken}
              </div>
              <Button onClick={() => copyToClipboard(newToken!)} className="w-full bg-blue-600 hover:bg-blue-700">
