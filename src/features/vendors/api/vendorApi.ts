...
import {type Vendor, type ModelFamily, type FamilyDetail} from "../types/vendor.types";
...
export const vendorApi = {
  ...
  // 🌟 Family 상세 조회 API 추가
  getFamilyDetail: async (familyId: number) => {
    const {data} = await apiClient.get<ApiResponse<FamilyDetail>>(`/api/v1/info/families/${familyId}`);
    return data.data;
  },
};
